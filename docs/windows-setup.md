# SO-101 on Windows — setup, calibration, and teleoperation

Operational guide for bringing up the SO-101 follower + leader arms on Windows:
environment install, COM port discovery, calibration, the STS3215 motor-ID fix,
teleoperation, and dataset recording.

The results this hardware produced are in the [main README](../README.md).
...
git clone https://github.com/aakashvardhan/so101-lerobot.git
cd so101-lerobot
uv sync --extra so101
```
