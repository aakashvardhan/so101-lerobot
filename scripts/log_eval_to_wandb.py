"""Log ACT real-robot evaluation results from the scoresheet to Weights & Biases.

Reads the manually-scored ACT_eval_scoresheet.xlsx (Fixed + Random tabs) and
pushes summary metrics into the training run so success rate lives next to the
loss curve. Success is human-scored, so it is computed here from the raw
Grasped?/Placed? cells rather than Excel's cached formulas.
...
# Per-trial layout in each tab: header on row 14, trials on rows 15-64.
FIRST_TRIAL_ROW = 15
LAST_TRIAL_ROW = 64
COL_GRASPED = "C"
COL_PLACED = "D"
...
def compute_metrics(xlsx_path, sheet_name):
    ...
    for row in range(FIRST_TRIAL_ROW, LAST_TRIAL_ROW + 1):
        g = _yn(cells.get(f"{COL_GRASPED}{row}"))
        p = _yn(cells.get(f"{COL_PLACED}{row}"))
        fail = cells.get(f"{COL_FAILURE}{row}")
        err = cells.get(f"{COL_ERR}{row}")
...
def main():
    ...
    run.log({"eval/summary_table": table,
             "eval/success_rate_bar": wandb.plot.bar(
                 table, "scenario", "success_rate", title="ACT success rate")})
    run.finish()
